...
def load_data():
    ...
    XY_mat = np.loadtxt('training_data/coordinate.txt')
    EToV = np.loadtxt('training_data/EToV.txt')
    data_test = np.loadtxt('training_data/initial_cond_0.txt').T.reshape(1,401,1536)
    data_training = np.zeros((49,401,1536))
    for ic in range(1,50):
        data_i = np.loadtxt('training_data/initial_cond_' + str(ic) + '.txt').T
